...
MODEL_CONNECTION_ERROR_MESSAGE = "模型服务连接失败，请检查模型网关、代理或网络配置"  # 外部模型网络异常的前端提示
MODEL_CONNECTION_EXCEPTIONS = (APIConnectionError, APITimeoutError, httpx.ConnectError, httpx.ConnectTimeout)
...
def _build_sse_error_event(message: str) -> str:
    """构造统一的 SSE 错误事件。"""
...
def _stream_agent(
    ...
    try:
        ...
        yield f"event: done\ndata: {done_payload}\n\n"
    # 捕获模型连接异常，避免第三方网关或代理不可用时穿透 StreamingResponse。
    except MODEL_CONNECTION_EXCEPTIONS as exc:
        # 记录模型连接失败日志，保留堆栈给服务端排障，前端只拿到稳定中文提示。
        logger.error(
            "[接口] Agent模型连接失败 追踪编号=%s 用户编号=%s 会话编号=%s 错误=%s",
            trace_id,
            user_id,
            conversation_id,
            exc,
        ...
        yield _build_sse_error_event(MODEL_CONNECTION_ERROR_MESSAGE)
    # 捕获接口异常、运行时异常和参数异常，避免流式连接直接中断无响应。
        ...
        yield _build_sse_error_event(str(exc))
...
def _stream_direct_rag(
    ...
    try:
        ...
        logger.error(
            "[聊天路由] 知识直答模型连接失败 追踪编号=%s 用户编号=%s 会话编号=%s 错误=%s",
            trace_id,
            user_id,
            conversation_id,
            exc,
