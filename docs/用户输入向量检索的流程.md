...
flowchart TD
    ...
    K --> L{"召回质量是否达标"}
    L -- "达标" --> M["跳过 LLM 拆分"]
    L -- "不达标" --> N{"是否极低分且无Collection领域关键词"}
...
现在 adaptive 模式多了一层保护：如果首次召回分数极低，并且当前问题没有命中当前 Collection 的领域关键词，就跳过 LLM Query Planner。
...
3. 用户问题没有命中当前 Collection 的领域关键词
4. top1_score < adaptive_skip_planner_max_score
5. top3_avg_score < adaptive_skip_planner_top3_avg_score
```

领域关键词来自字典表 `collection_domain_keyword`，可按 `metadata.collection_name` 绑定到不同 Collection。
默认 `agent` Collection 初始化了一组扫地机器人相关领域词；后续换知识领域时，调整字典项即可，不需要修改 RAG 代码。
...
| `[查询规划] adaptive模式跳过模型改写` | `rag/rag_service.py` | 极低分且无 Collection 领域关键词时跳过 Planner |
...
3. 如果召回质量不达标，会先判断是否“极低分且无 Collection 领域关键词”。
4. 该问题命中了 `agent` Collection 的领域关键词，如“宠物、地毯、充电、基站”，所以不会因为极低分直接跳过 Planner。
5. 调用 LLM Query Planner 生成多个 search_query。
6. 合并原问题和新增 query。
7. 新增 query 并行检索。
