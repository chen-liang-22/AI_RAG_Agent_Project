...
现在 adaptive 模式多了一层保护：如果首次召回分数极低，并且当前问题没有命中扫地机器人业务关键词，就跳过 LLM Query Planner。
...
业务关键词包括：扫地、扫拖、机器人、清扫、地毯、宠物、充电、基站、地图、建图、避障、尘盒、拖布、滚刷、故障、报错、联网、续航等。
...
| `[查询规划] adaptive模式跳过模型改写` | `rag/rag_service.py` | 极低分且无业务关键词时跳过 Planner |
...
3. 如果召回质量不达标，会先判断是否“极低分且无业务关键词”。
4. 该问题命中了业务关键词，如“宠物、地毯、充电、基站”，所以不会因为极低分直接跳过 Planner。
