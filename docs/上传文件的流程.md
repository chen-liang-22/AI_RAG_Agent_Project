...
flowchart TD
    ...
    W --> X["DocumentParser 切分 segments/QA"]
...
当前上传设计的主干是合理的：两阶段上传让用户先预览和确认，SQLite 与 Qdrant 的职责边界清晰，`document_id` 贯穿文件管理、删除、重建和检索 payload，已经具备知识库管理系统的基础形态。
...
- 如果删除成功后，embedding 或 Qdrant 写入失败，该文件会变成 SQLite 中存在但 Qdrant 中不可检索。
