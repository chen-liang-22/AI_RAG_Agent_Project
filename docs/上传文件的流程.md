...
注意：当前删除会同时处理三部分：Qdrant points 删除、MySQL 文件状态变为 `deleted`、MinIO 原始对象物理删除。
...
- 预览文件保存在 MinIO 的 `previews/{upload_id}/{filename}`。
...
- 后续可增加定时清理任务，删除过期的 MinIO `previews/` 临时对象。
- 清理前只允许删除 MinIO `previews/` 前缀下的临时对象，避免误删正式文件。
...
当前删除会删除 Qdrant points、标记 MySQL 状态，并删除 MinIO 原始对象。后续可以按业务需要增加：
...
| 临时对象保存 | 500 保存失败 | 检查 MinIO 桶权限、网络和磁盘空间 |
