...
```mermaid
flowchart TB
  API["api/main.py<br/>FastAPI 启动入口"] --> Router["app_v2/api/router.py<br/>V2 路由总入口"]
  Router --> Routes["app_v2/api/routes/*<br/>HTTP 路由层"]
  Routes --> App["app_v2/application/*<br/>应用服务层"]
  App --> Domain["app_v2/domain/*<br/>实体、DTO、常量"]
  App --> Repo["app_v2/infrastructure/repositories/*<br/>MySQL 仓储"]
  App --> Adapter["app_v2/infrastructure/adapters/*<br/>MinIO/Qdrant 适配器"]
