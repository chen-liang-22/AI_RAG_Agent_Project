...
系统当前已经进入 V2 分层架构：后端主线在 `app_v2`，前端主线在 `src/features`。旧的 `core` 模块仍承载 RAG、模型工厂、文件处理、切片策略和旧 Agent 能力，属于底层能力层，不直接承担页面业务编排。
...
flowchart LR
  ...
  H --> R["FastAPI 路由层<br/>app_v2/api/routes"]
  R --> A["应用服务层<br/>app_v2/application"]
  A --> D["领域对象与接口 DTO<br/>app_v2/domain"]
  A --> I["基础设施层<br/>app_v2/infrastructure"]
...
后端使用 FastAPI，入口是 `api/main.py`。V2 接口统一挂在 `/api/v2`，路由总入口是 `app_v2/api/router.py`。

| 层级 | 目录 | 主要职责 |
| --- | --- | --- |
| API 启动层 | `api/` | 创建 FastAPI app、启动预热、挂载 V2 路由 |
| 路由层 | `app_v2/api/routes/` | 接收 HTTP 请求、参数校验、调用应用服务 |
| 应用服务层 | `app_v2/application/` | 编排业务流程，例如上传、聊天、考试、训练、登录 |
| 领域层 | `app_v2/domain/` | ORM 实体、接口 DTO、常量 |
| 基础设施层 | `app_v2/infrastructure/` | MySQL 会话、仓储、MinIO、Qdrant 适配器、ID 生成 |
...
2. `core` 仍包含旧 Agent 和 RAG 能力，是底层能力仓库，不建议继续在里面堆业务编排。新增业务应优先落在 `app_v2/application`。
