...
| 后端 `app_v2/api/routes/training.py` | 暴露 `/api/v2/training/knowledge/` 系列接口 |
| 后端 `app_v2/application/training/material_service.py` | 训练资料外观服务，统一收口上传、预览、发布、删除等动作 |
| 后端 `app_v2/application/training/sales_training_core.py` | 训练资料核心编排，负责 MinIO、MySQL、Qdrant、LLM 兜底切片 |
...
flowchart TB
  ...
  H --> I["查询已发布批次<br/>get_published_batch_by_md5"]

  I --> J{"是否已存在相同 MD5 的已发布资料"}
