...
def test_outline_qa_parser_handles_pdf_whitespace_inside_titles():
    parser = DocumentParser(
        RecursiveCharacterTextSplitter(
            chunk_size=200,
            chunk_overlap=20,
    ...
    outline = [
        ...
        {"level": 1, "title": "1、ArrayList和LinkedList的区别", "page": 1},
        {"level": 1, "title": "2、HashMap和Hashtable的区别", "page": 1},
        {"level": 1, "title": "3、ConcurrentHashMap的原理", "page": 1},
        {"level": 0, "title": "JVM面试题", "page": 2},
        {"level": 1, "title": "1、什么是类加载器？", "page": 2},
        {"level": 1, "title": "2、什么是双亲委派？", "page": 2},
        {"level": 1, "title": "3、GC Roots有哪些？", "page": 2},
    ]
    documents = [
        Document(
            page_content=(
                "Java集合面试题\n"
                "1、ArrayList 和\nLinkedList 的区别\n"
                "答案：ArrayList 基于数组，LinkedList 基于链表。\n"
                "2、HashMap 和\nHashtable 的区别\n"
                "答案：HashMap 允许 null，Hashtable 线程安全。\n"
                "3、ConcurrentHashMap 的\n原理\n"
                "答案：它通过 CAS 和分段降低锁竞争。"
            ),
            metadata={"page": 0, "source": "Java面试题.pdf", "_pdf_outline": outline},
        ),
        Document(
            page_content=(
                "JVM面试题\n"
                "1、什么 是\n类加载器？\n"
                "答案：类加载器负责加载 class 文件。\n"
                "2、什么 是 双亲 委派？\n"
                "答案：优先委托父加载器加载类。\n"
                "3、GC Roots 有哪些？\n"
                "答案：包括栈帧引用和静态变量引用。"
            ),
            metadata={"page": 1, "source": "Java面试题.pdf"},
        ),
    ]

    segments, qa_items = parser.build_segments_and_qas(
        document_id="doc_java_space",
        documents=documents,
        document_type="qa",
        split_strategy="outline_qa",
    )
    ...
    assert len(segments) == 6
    assert len(qa_items) == 6
    assert qa_items[0].answer == "ArrayList 基于数组，LinkedList 基于链表。"
    assert qa_items[0].metadata["outline_match_method"] == "compact"
    assert "ArrayList 和\nLinkedList 的区别" not in qa_items[0].answer
    assert qa_items[3].answer == "类加载器负责加载 class 文件。"


def test_outline_qa_parser_prefers_outline_page_over_toc_page():
    parser = DocumentParser(
        RecursiveCharacterTextSplitter(
            chunk_size=200,
            chunk_overlap=20,
    ...
    outline = [
        {"level": 0, "title": "JVM基础", "page": 2},
        {"level": 1, "title": "1、什么是JVM？", "page": 2},
        {"level": 1, "title": "2、什么是JDK？", "page": 2},
        {"level": 1, "title": "3、什么是JRE？", "page": 2},
        {"level": 0, "title": "并发基础", "page": 3},
        {"level": 1, "title": "1、什么是线程？", "page": 3},
        {"level": 1, "title": "2、什么是线程池？", "page": 3},
        {"level": 1, "title": "3、什么是锁？", "page": 3},
    ]
    documents = [
        Document(
            page_content=(
        ...
        Document(
            page_content=(
                "JVM基础\n"
                "1、什么是JVM？\n"
                "答案：JVM 是运行 Java 字节码的虚拟机。\n"
                "2、什么是JDK？\n"
                "答案：JDK 包含编译和运行 Java 程序需要的工具。\n"
                "3、什么是JRE？\n"
                "答案：JRE 提供 Java 程序运行环境。"
            ),
            metadata={"page": 1, "source": "Java面试题.pdf"},
        ),
        Document(
    ...
    assert len(segments) == 6
    assert len(qa_items) == 6
    assert qa_items[0].answer == "JVM 是运行 Java 字节码的虚拟机。"
    assert "什么是JDK" not in qa_items[0].answer
    assert qa_items[3].answer == "线程是程序执行的基本调度单位。"
