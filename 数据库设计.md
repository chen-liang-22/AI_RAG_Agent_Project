...
本文档整理当前项目的数据存储设计，包含 SQLite 业务数据库、Qdrant 向量库、两者职责边界、核心字段中文注释、入库流程和检索流程。
...
| --- | --- | --- | --- |
| 关系型数据库 | SQLite，默认文件为 `storage/knowledge.db` | 保存业务状态、文件元数据、会话历史、系统字典、考试记录 | 不保存知识正文作为问答检索来源 |
| 向量数据库 | Qdrant，默认 collection 为 `agent` | 保存知识分片、embedding 向量、检索 metadata | 不保存会话状态、用户操作流水、业务字典 |

设计重点：

1. 知识问答主链路只依赖 Qdrant 召回，不再使用 SQLite 对 FAQ 或知识正文做 `LIKE` 查询。
2. SQLite 只保存“可管理、可追踪、可展示”的业务数据，例如文件上传状态、会话记录、考试记录。
...
| --- | --- |
| 数据库类型 | SQLite |
| 默认路径 | `storage/knowledge.db` |
| 初始化入口 | `rag/knowledge_store.py` 中的 `KnowledgeStore.init_db()` |
| 连接方式 | 每次通过 `KnowledgeStore.connect()` 打开连接，开启 `PRAGMA foreign_keys = ON` |
| 时间格式 | 文本格式 ISO 时间字符串，由 `utc_now_text()` 统一生成 |
...
| `file_path` | `TEXT` | 是 | 文件在服务端本地的保存路径。预览接口读取原文件时使用。 |
...
表含义：  
保存一次知识掌握度测试的主记录。题源来自 Qdrant 中的结构化 QA 分片，考试过程和结果保存在 SQLite。
...
| `document_segments` | 保存文档分片文本 | 分片正文如果同时存在 SQLite 和 Qdrant，会出现双写一致性问题。 |
| `knowledge_units` | 保存知识单元 | 结构过早固定，容易限制不同类型文档；当前由 Qdrant metadata 承担可检索结构。 |

当前原则：  
知识正文、FAQ、文档分片和可检索 payload 以 Qdrant 为准。SQLite 不再作为知识答案来源。

### 2.10 SQLite 系统表说明

`sqlite_master` 是 SQLite 自带系统表，不是项目业务表。

| 表名 | 作用 |
| --- | --- |
| `sqlite_master` | 保存当前 SQLite 数据库中的表、索引、视图、触发器等 schema 信息。执行 `SELECT * FROM sqlite_master;` 可以查看建表语句和索引定义。 |

SQLite 本身不支持像 MySQL `COMMENT ON TABLE` 或字段 comment 那样直接给表和列写中文备注。因此项目表注释主要通过本文档、代码注释和接口文档维护。
...
| metadata 字段 | 中文注释 |
| --- | --- |
| `document_id` | 文件业务编号。与 SQLite `documents.document_id` 对应，用于按文件删除向量、重建索引、追踪来源。 |
...
flowchart TD
    ...
    J --> K["更新 SQLite documents 状态和 chunk_count"]
```
...
flowchart TD
    ...
    H --> I["把参考资料交给最终回答模型"]
    I --> J["模型按客服口吻润色回答"]
    J --> K["保存 user/assistant 消息到 SQLite"]
```
...
| 操作 | 处理方式 |
| --- | --- |
| 删除单个文件 | 根据 `metadata.document_id` 删除 Qdrant 中该文件所有 points，同时更新 SQLite 文件状态。 |
| 重建单个文件索引 | 重新读取原文件，重新切分，先删旧 points，再写新 points。 |
| 全量重建 collection | 删除并重建当前 collection，然后遍历 SQLite `documents` 中有效文件重新写入。 |
| 重复上传文件 | 通过 SQLite `documents.file_md5` 判断是否已有相同文件。 |
...
| 设计 | 问题 |
| --- | --- |
| 把 FAQ 问题全文存 SQLite 后用 `LIKE` 查询 | 用户换个说法就可能查不到，召回能力弱。 |
| SQLite 和 Qdrant 同时保存同一份分片正文 | 双写一致性复杂，重建索引容易产生脏数据。 |
...
| 模式 | 使用位置 | 为什么使用 |
| --- | --- | --- |
| 外观模式 | `KnowledgeStore`、`VectorStoreService` | 对外隐藏 SQLite 操作、Qdrant 操作、文件解析和索引细节，让 API 层调用更简单。 |
...
| 方向 | 建议 |
| --- | --- |
| 表结构文档化 | SQLite 不支持原生中文 comment，建议继续维护本文档，并在建表代码旁保留关键注释。 |
...
这个项目的数据设计应该坚持：SQLite 管业务状态，Qdrant 管知识检索。用户问答时优先走 Qdrant 语义召回和精排，再交给大模型生成自然回答；SQLite 只负责记录文件、会话和业务过程，不再承担知识答案查询职责。
